...
- **메시지 전달** — 다른 채널로 메시지를 전달하고 원본 출처 표시
- **읽음 표시** — 마지막으로 보낸 메시지가 몇 명에게 읽혔는지 실시간 확인
- **드래그앤드롭 · 클립보드 붙여넣기** — 채팅창에 파일을 끌어놓거나 스크린샷을 바로 붙여넣어 첨부
...
- **채널 즐겨찾기** — 자주 쓰는 채널을 사이드바 상단에 고정
- **프로필 관리** — 이름·부서 수정과 비밀번호 변경
- **채널 정보 패널** — 고정 메시지, 공유 노트, 체크리스트를 한 곳에서 확인하는 우측 패널
- **아이콘 레일 내비게이션** — 검색·출퇴근·다크모드·프로필을 모아둔 좌측 고정 내비게이션 바
- **랜딩 페이지** — 로그인 전 제품 소개, 기능, 요금제(예시)를 보여주는 마케팅 화면
...
| `POST` | `/api/channels/:id/favorite` | 채널 즐겨찾기 설정 |
| `GET` | `/api/channels/:id/notes` | 채널 공유 노트 조회 |
| `PUT` | `/api/channels/:id/notes` | 채널 공유 노트 수정 |
| `GET` | `/api/channels/:id/checklist` | 채널 체크리스트 조회 |
| `POST` | `/api/channels/:id/checklist` | 체크리스트 항목 추가 |
| `PATCH` | `/api/channels/:id/checklist/:itemId` | 체크리스트 항목 완료 토글 |
| `DELETE` | `/api/channels/:id/checklist/:itemId` | 체크리스트 항목 삭제 |
...
| Client → Server | `message:forward` | 메시지를 다른 채널로 전달 |
...
| Server → Client | `channel:noteUpdated` | 채널 공유 노트 갱신 알림 |
| Server → Client | `channel:checklistUpdated` | 채널 체크리스트 갱신 알림 |
...
| `channel_notes` | 채널별 공유 노트 |
| `channel_checklist_items` | 채널별 체크리스트 항목 |
